...
class LiveTradingEngine:
    def __init__(self, config: Config):
        ...
        if DCAManager:
            try:
                ...
                self.logger.info("🔄 [DCA] DCA integration enabled - will initialize with Binance client")
            except Exception as e:
        ...
        if LSTMPredictor and getattr(config, "lstm_enable", False):
            try:
                ...
                self.logger.info("LSTM predictor initialized for enhanced signal generation")
            except Exception as e:
        ...
        self.use_trailing_stop = bool(getattr(config, "use_trailing_stop", True))  # Enabled by default
        if self.use_trailing_stop:
            self.logger.info("Trailing stop loss enabled - will initialize with Binance client")
        ...
        self.emergency_stop_loss_pct = float(getattr(config, "emergency_stop_loss_pct", 20.0))
        self.logger.info(f"Emergency Stop Loss enabled: -{self.emergency_stop_loss_pct}% equity loss")
        ...
        self.logger.info("Live trading engine initialized with DCA and LSTM support")
    ...
    def _position_size_qty(self, price: Optional[float], strength: float = 0.5) -> Optional[float]:
        ...
        self.logger.info("[POSITION_SIZE] Equity=%.2f, Risk=%.2f%%, Strength=%.2f (clamped=%.2f), Value=%.2f (max=%.2f), Qty=%.6f", 
                        equity, risk_pct, strength, strength_clamped, final_value, max_position, qty)
    ...
    async def _initialize_market_context(self) -> None:
        ...
        try:
            ...
            self.market_context_manager = MarketContextManager(self.config, data_fetcher)
            ...
            if context:
                ...
                logger.info(f"📊 [ADAPTIVE_THRESHOLD] Original: {original_threshold:.3f} → Recommended: {recommended_threshold:.3f}")
                
                # Apply position size multiplier
                original_risk = self.risk_pct
                adjusted_risk = original_risk * context.suggested_position_size_multiplier
                self.risk_pct = max(0.1, min(2.0, adjusted_risk))  # Cap between 0.1% and 2%
                
                logger.info(f"📊 [ADAPTIVE_SIZING] Risk per trade: {original_risk:.2f}% → {self.risk_pct:.2f}%")
                ...
                if hasattr(self.signaler, 'apply_market_context'):
                    try:
                        ...
                        logger.info("🎯 [SIGNAL_TUNING] Applied market context to signal generator")
                    except Exception as sg_e:
                        logger.warning(f"🎯 [SIGNAL_TUNING] Failed to apply context to signals: {sg_e}")
                
                # Log risk warnings
                if context.risk_warning_level in ["HIGH", "CRITICAL"]:
                    logger.warning(f"⚠️ [RISK_WARNING] Market risk level: {context.risk_warning_level}")
                    logger.warning("⚠️ [RISK_WARNING] Consider reduced position sizes or pausing trading")
                
                logger.info("✅ [MARKET_CONTEXT] Market analysis complete - trading recommendations applied")
            else:
                logger.warning("❌ [MARKET_CONTEXT] No context data received - using default settings")
            ...
            if self.learning_enabled:
                try:
                    ...
                    adaptive_params = self.adaptive_learning.get_current_adaptive_params()
                    if adaptive_params:
                        # Override settings with learned parameters
                        if adaptive_params.get('confidence_threshold'):
                            logger.info(f"🧠 [LEARNING] Applying learned confidence threshold: {adaptive_params['confidence_threshold']:.3f}")
                            
                        if adaptive_params.get('position_size_multiplier'):
                            original_risk = self.risk_pct
                            learned_risk = original_risk * adaptive_params['position_size_multiplier']
                            self.risk_pct = max(0.1, min(2.0, learned_risk))
                            logger.info(f"🧠 [LEARNING] Applying learned position sizing: {original_risk:.2f}% → {self.risk_pct:.2f}%")
                            
                        # Log learning status
                        if adaptive_params.get('is_ab_testing'):
                            logger.info(f"🧪 [A/B_TEST] Currently running variant {adaptive_params.get('ab_variant')}")
                    
                    # 🧠 Initialize Advanced AI features if available
                    await self._initialize_advanced_ai()
                    
                    # 📊 FORCE INITIAL DASHBOARD GENERATION
                    logger.info("📊 [DASHBOARD_CHECK] Checking dashboard initialization...")
                    if self.adaptive_learning:
                        logger.info("📊 [DASHBOARD_CHECK] ✅ adaptive_learning exists")
                        
                        if hasattr(self.adaptive_learning, 'learning_visualizer'):
                            logger.info("📊 [DASHBOARD_CHECK] ✅ learning_visualizer attribute exists")
                            
                            if self.adaptive_learning.learning_visualizer:
                                logger.info("📊 [DASHBOARD_CHECK] ✅ learning_visualizer is initialized")
                                
                                try:
                                    logger.info("📊 [DASHBOARD] Force generating initial dashboard with current data...")
                                    ...
                                    dashboard_path = await self.adaptive_learning.learning_visualizer.create_learning_dashboard()
                                    if dashboard_path:
                                        logger.info(f"📊 [DASHBOARD] ✅ Initial dashboard created: {dashboard_path}")
                                        logger.info(f"📊 [DASHBOARD] 🌐 Open in browser: file://{Path(dashboard_path).absolute()}")
                                    else:
                                        logger.warning("📊 [DASHBOARD] ❌ Dashboard creation returned empty path")
                                        
                                except Exception as dash_e:
                                    logger.error(f"📊 [DASHBOARD] ❌ Failed to generate initial dashboard: {dash_e}")
                                    import traceback
                                    logger.error(f"📊 [DASHBOARD] Traceback: {traceback.format_exc()}")
                            else:
                                logger.warning("📊 [DASHBOARD_CHECK] ❌ learning_visualizer is None")
                        else:
                            logger.warning("📊 [DASHBOARD_CHECK] ❌ learning_visualizer attribute missing")
                    else:
                        logger.warning("📊 [DASHBOARD_CHECK] ❌ adaptive_learning is None")
                            
                except ImportError:
                    logger.warning("🤖 [ADAPTIVE_LEARNING] AdaptiveLearningSystem not available")
                    self.adaptive_learning = None
                except Exception as learning_e:
                    logger.error(f"🤖 [ADAPTIVE_LEARNING] Failed to initialize: {learning_e}")
                    self.adaptive_learning = None
            else:
                logger.info("🤖 [ADAPTIVE_LEARNING] Learning disabled by configuration")
                
        except Exception as e:
            logger.error(f"❌ [MARKET_CONTEXT] Failed to initialize market context: {e}")
            logger.warning("⚠️ [MARKET_CONTEXT] Continuing with default trading settings")
    ...
    async def _preload_historical_data(self, limit: int) -> None:
        ...
        self.logger.info("[PRELOAD] Loading %d historical candles for IMBA signals...", limit)
        
        for symbol in self.symbols:
            try:
                self.logger.info("[PRELOAD] Loading %d candles for %s...", limit, symbol)
                
                # Use market data provider to fetch historical candles
                if self.market and hasattr(self.market, "get_candles"):
                    candles = await self.market.get_candles(symbol, self.timeframe, limit=limit)
                    
                    if candles and len(candles) > 0:
                        self.logger.info("[PRELOAD] SUCCESS: Loaded %d candles for %s", len(candles), symbol)
                        ...
                        try:
                            ...
                            if hasattr(candles, 'close'):
                                ...
                                try:
                                    ...
                                    if not valid_mask.all():
                                        ...
                                        self.logger.warning(f"[PRELOAD] {symbol}: Filtered {invalid_count} invalid timestamps")
                                        df = df[valid_mask]
                                    ...
                                    range_mask = (df['timestamp'] >= min_timestamp) & (df['timestamp'] <= max_timestamp)
                                    if not range_mask.all():
                                        out_of_range = (~range_mask).sum()
                                        self.logger.warning(f"[PRELOAD] {symbol}: Filtered {out_of_range} out-of-range timestamps")
                                        df = df[range_mask]
                                    
                                    if df.empty:
                                        self.logger.error(f"[PRELOAD] {symbol}: No valid timestamps after filtering")
                                        df = None
                                    else:
                                    ...
                                    self.logger.error(f"[PRELOAD] {symbol}: Timestamp conversion failed: {e}")
                                    df = None
                                    ...
                                    try:
                                        for col in ['open', 'high', 'low', 'close', 'volume']:
                                            ...
                                            if invalid_mask.any():
                                                ...
                                                self.logger.warning(f"[PRELOAD] {symbol}: {col} has {invalid_count} invalid values, forward filling")
                                                
                                                # Forward fill invalid values
                                                df[col] = df[col].fillna(method='ffill')
                                                df[col] = df[col].replace([float('inf'), float('-inf')], method='ffill')
                                                ...
                                                if col == 'volume' and df[col].isna().any():
                                                    ...
                                                    df[col] = df[col].fillna(df[col].iloc[-1] if len(df) > 0 else 1.0)
                                        
                                        # Final validation: ensure all data is finite
                                        numeric_cols = ['open', 'high', 'low', 'close', 'volume']
                                        valid_data_mask = df[numeric_cols].apply(lambda x: x.notna() & np.isfinite(x)).all(axis=1)
                                        
                                        if not valid_data_mask.all():
                                            invalid_rows = (~valid_data_mask).sum()
                                            self.logger.warning(f"[PRELOAD] {symbol}: Removing {invalid_rows} rows with invalid data")
                                            df = df[valid_data_mask]
                                        
                                        if df.empty:
                                            self.logger.error(f"[PRELOAD] {symbol}: No valid data after cleaning")
                                            df = None
                                            
                                    except Exception as e:
                                        self.logger.error(f"[PRELOAD] {symbol}: Numeric conversion failed: {e}")
                                        df = None
                            else:
                                df = None
                            ...
                            if df is not None and not df.empty:
                                ...
                                if hasattr(self.signaler, '_historical_data'):
                                    if not isinstance(self.signaler._historical_data, dict):
                                    ...
                                    try:
                                        ...
                                        if len(df) >= 3:
                                            ...
                                            latest_idx = -1  # Use last (most recent) candle instead of first (oldest)
                                        ...
                                        self.logger.info(f"[PRELOAD] ✅ VERIFIED DATA LOADED for {symbol}:")
                                        self.logger.info(f"[PRELOAD]   📊 Records: {len(df)} candles")
                                        self.logger.info(f"[PRELOAD]   📅 Period: {first_ts} → {last_ts}")  
                                        self.logger.info(f"[PRELOAD]   💾 Memory: {memory_mb:.2f} MB")
                                        self.logger.info(f"[PRELOAD]   🎯 {sample_data}")
                                        self.logger.info(f"[PRELOAD]   ✅ CACHED: Ready for IMBA analysis")
                                        
                                    except Exception as detail_error:
                                        self.logger.info("[PRELOAD] SUCCESS: Stored %d candles as DataFrame in cache for %s", 
                                                       len(df), symbol)
                                        self.logger.debug(f"[PRELOAD] Detail logging failed: {detail_error}")
                                else:
                                    self.logger.warning("[PRELOAD] SignalGenerator has no _historical_data attribute")
                            else:
                                self.logger.warning("[PRELOAD] Failed to convert candles to DataFrame for %s", symbol)
                                
                        except Exception as e:
                            self.logger.error("[PRELOAD] Failed to convert candles to DataFrame: %s", e)
                            import traceback
                            self.logger.error(traceback.format_exc())
                    else:
                        self.logger.warning("[PRELOAD] WARNING: No candles returned for %s", symbol)
                else:
                    self.logger.warning("[PRELOAD] WARNING: MarketDataProvider not available, skipping preload")
                    
            except Exception as e:
                self.logger.error("[PRELOAD] ERROR: Failed to load candles for %s: %s", symbol, e)
    ...
    async def _run_trading_loop(self) -> None:
        self.logger.info("Starting main trading loop for %d symbols: %s", len(self.symbols), ", ".join(self.symbols))
        ...
        while self.running:
            ...
            try:
                ...
                if await self._check_emergency_stop_loss():
                    self.logger.critical("🚨 EMERGENCY STOP LOSS TRIGGERED! Halting all trading operations.")
                    self.running = False
                    break
                ...
                if self.adaptive_learning and self.iteration % 20 == 0:
                    try:
                        ...
                        self.logger.debug("[AI_CYCLE] Failed to run AI optimization: %s", ai_e)
                
                # 📊 Generate learning visualization reports (every 60 iterations ~ 60 seconds)
                if self.adaptive_learning and hasattr(self.adaptive_learning, 'learning_visualizer') and self.adaptive_learning.learning_visualizer and self.iteration % 60 == 0:
                    try:
                        await self._generate_learning_visualization()
                    except Exception as viz_e:
                        self.logger.debug("[LEARNING_VIZ] Failed to generate visualization: %s", viz_e)
                ...
                if self.portfolio_tracker and self.iteration % 60 == 0:
                    try:
                        ...
                        self.logger.debug("[PORTFOLIO] Failed to log portfolio summary: %s", pt_e)
    ...
    async def _process_symbol(self, symbol: str) -> None:
        ...
        if self.market and hasattr(self.market, "get_candles"):
            try:
                md = await self.market.get_candles(symbol, self.timeframe, limit=250)  # IMBA needs 250+ candles
            except Exception as e:
        ...
        if not sig:
            ...
            self.logger.debug("[SYMBOL_CHECK] %s: No actionable signal (wait/rejected)", symbol)
        ...
        if not self.dry_run:
            try:
                ...
                if has_position:
                    self.logger.info("[POSITION_EXISTS] Already have %s position for %s, skipping new entry", sig.side, symbol)
                    ...
                    if self.dca_manager and symbol in self.active_positions:
                        try:
                            ...
                            pnl_pct = (pnl / (current_position.get('entry_price', 1) * current_position.get('quantity', 1))) * 100
                            self.logger.info("[POSITION_STATUS] %s: PnL=$%.2f (%.2f%%), checking DCA conditions...", 
                                           symbol, pnl, pnl_pct)
                            
                            should_dca = await self.dca_manager.should_dca(symbol, current_position)
                            if should_dca:
                                self.logger.info("[DCA_TRIGGER] Conditions met for DCA on %s", symbol)
                                await self._handle_dca_trigger(symbol, current_position)
                            else:
                                self.logger.debug("[DCA_CHECK] No DCA needed for %s (PnL: %.2f%%)", symbol, pnl_pct)
                ...
                has_opposite = await self._check_existing_position(symbol, opposite_side)
                if has_opposite:
                    self.logger.warning("[POSITION_CONFLICT] ⚠️  Signal %s but have %s position for %s - BLOCKING TRADE!", 
                                       sig.side, opposite_side, symbol)
                    self.logger.warning("[POSITION_CONFLICT] Refusing to enter against existing position direction")
                    return
                ...
                if symbol not in self.active_positions:
                    ...
                    if self.dca_manager and hasattr(self.dca_manager, 'start_dca_position'):
                        try:
                            from core.types import OrderSide
                            order_side = OrderSide.BUY if sig.side == "BUY" else OrderSide.SELL
                            # CRITICAL FIX: Handle both sync and async DCA methods
                            result = self.dca_manager.start_dca_position(symbol, order_side, price, f"Signal strength: {sig.strength:.2f}")
                            if asyncio.iscoroutine(result):
                                await result
                            self.logger.info("[DCA] Started DCA position for %s", symbol)
                        except Exception as dca_e:
                            self.logger.warning("[DCA] Failed to start DCA position: %s", dca_e)
    ...
    async def _place_order(self, symbol: str, side: str, qty: float, price: float, strength: float) -> None:
        ...
        try:
            ...
            if notional < self.min_notional:
                self.logger.warning("[ORDER_SKIP] Notional %.2f < min %.2f USDT", notional, self.min_notional)
                return
            ...
            if rounded_qty < min_qty:
                rounded_qty = min_qty
                self.logger.info("[ORDER] Quantity %.3f too small, using minimum %.3f", round(qty, 3), min_qty)
            
            # Get symbol-specific decimal places for accurate logging
            precision_map = {
                "BTCUSDT": 3, "ETHUSDT": 3, "BNBUSDT": 2, "SOLUSDT": 2,
                "XRPUSDT": 1, "ADAUSDT": 0, "DOGEUSDT": 0, "AVAXUSDT": 0,  # FIXED: AVAX needs 0 decimals like ADA/DOGE
                "LINKUSDT": 1, "MATICUSDT": 0,
            }
            decimal_places = precision_map.get(symbol, 3)
            qty_format = f"{{:.{decimal_places}f}}"
            
            self.logger.info("[ORDER] Placing %s order: %s %s @ %.2f (notional: %.2f USDT)", 
                           order_side, qty_format.format(rounded_qty), symbol, price, notional)
            ...
            if hasattr(self, 'client') and self.client:
                try:
                    ...
                    try:
                        ...
                        if slippage_pct > 2.0:
                            self.logger.warning(f"🚨 [HIGH_SLIPPAGE] Expected: ${price:.4f}, Market: ${current_market_price:.4f}, Slippage: {slippage_pct:.2f}%")
                            
                            # SAFETY: Block trade if slippage is extreme (>5%)
                            if slippage_pct > 5.0:
                                self.logger.error(f"❌ [EXTREME_SLIPPAGE] Blocking trade due to {slippage_pct:.2f}% slippage (>5% threshold)")
                        ...
                        self.logger.info(f"[ORDER_PRICE] Signal: ${price:.4f}, Market: ${current_market_price:.4f}, Limit: ${limit_price:.4f}")
                        
                    except Exception as price_e:
                        self.logger.warning(f"[PRICE_CHECK] Failed to get market price: {price_e}, using MARKET order")
                        limit_price = None
                    ...
                    self.logger.info("[ORDER_RESULT] %s order %s: status=%s, executed=%.3f/%.3f", 
                                   order_side, order_id, order_status, executed_qty, rounded_qty)
                    
                    # CRITICAL DEBUG: Log exact values for debugging
                    self.logger.info("[ORDER_DEBUG] status='%s', executed_qty=%s (type: %s), zero_check=%s", 
                                   order_status, executed_qty, type(executed_qty), executed_qty == 0)
                    ...
                    if order_status == 'NEW' and (executed_qty == 0 or executed_qty == 0.0 or executed_qty < 0.001):
                        self.logger.info("[ORDER_WAIT] Market order placed, waiting for execution...")
                        ...
                        for attempt in range(10):  # 10 attempts * 0.5s = 5 seconds max
                            ...
                            try:
                                ...
                                self.logger.debug("[ORDER_CHECK] Attempt %d: status=%s, executed=%.3f", 
                                                attempt+1, order_status, executed_qty)
                                
                                if order_status == 'FILLED' and executed_qty > 0:
                                    break
                            except Exception as check_e:
                                self.logger.warning("[ORDER_CHECK] Failed to check order %s: %s", order_id, check_e)
                                continue
                    ...
                    if order_status == 'FILLED' and executed_qty > 0:
                        ...
                        if fills:
                            ...
                            self.logger.info("[ORDER_FILLED] ✅ %s %.3f %s @ $%.2f (avg fill price)", 
                                           order_side, executed_qty, symbol, avg_fill_price)
                        else:
                            avg_fill_price = price  # Fallback to market price
                            
                        # Verify position was created
                        await self._verify_position_created(symbol, order_side, executed_qty)
                        
                        # Setup Take Profit and Stop Loss orders with actual fill data
                        await self._setup_tp_sl_orders(symbol, order_side, executed_qty, avg_fill_price, strength)
                        
                        # Register TP/SL orders with Exit Tracker if available
                        if self.exit_tracker and hasattr(self, 'pending_trades') and symbol in self.pending_trades:
                            try:
                                ...
                                if trade_id:
                                    ...
                                    self.logger.info("🎯 [EXIT_TRACKER] Will monitor position exits for trade %s", trade_id)
                            except Exception as et_e:
                                self.logger.warning("🎯 [EXIT_TRACKER] Failed to register orders: %s", et_e)
                        ...
                        if self.adaptive_learning:
                            try:
                                ...
                                tp_prices, sl_price = self._calculate_tp_sl_levels(avg_fill_price, order_side, strength)
                                ...
                                self.logger.info("🤖 [LEARNING] ✅ Trade recorded in AI system: %s %.3f @ %.4f", 
                                               order_side, executed_qty, avg_fill_price)
                            except Exception as learning_e:
                                self.logger.error(f"🤖 [LEARNING] ❌ Failed to record trade in AI system: {learning_e}")
                                import traceback
                                self.logger.error(f"🤖 [LEARNING] Traceback: {traceback.format_exc()}")
                        
                        self.logger.info("[ORDER_SUCCESS] ✅ %s position opened: %.3f %s", 
                                       order_side, executed_qty, symbol)
                    else:
                        # CRITICAL: If order still not filled, cancel it and exit
                        if order_status in ['NEW', 'PARTIALLY_FILLED']:
                            self.logger.warning("[ORDER_CANCEL] Cancelling unfilled order %s", order_id)
                            try:
                                self.client.cancel_order(symbol=symbol, orderId=order_id)
                                self.logger.info("[ORDER_CANCEL] ✅ Order %s cancelled", order_id)
                            except Exception as cancel_e:
                                self.logger.error("[ORDER_CANCEL] Failed to cancel order %s: %s", order_id, cancel_e)
                        
                        self.logger.error("[ORDER_FAILED] ❌ Order not filled: status=%s, executed=%.3f", 
                                        order_status, executed_qty)
                        if order_result:
                            self.logger.error("[ORDER_DETAILS] Full result: %s", order_result)
                        return  # Don't setup TP/SL for unfilled orders
                        
                except Exception as order_error:
                    self.logger.error("[ORDER_EXCEPTION] ❌ Failed to place %s order: %s", order_side, order_error)
                    # Try to get account info for debugging
                    try:
                        account = self.client.futures_account()
                        balance = account.get('totalWalletBalance', 'unknown')
                        self.logger.error("[ACCOUNT_DEBUG] Wallet balance: %s USDT", balance)
                    except Exception:
                ...
                self.logger.warning("[ORDER_SKIP] No Binance client available, order not placed")
                
        except Exception as e:
            self.logger.error("[ORDER_FAILED] Failed to place %s order for %s: %s", side, symbol, e)
            raise

    async def _verify_position_created(self, symbol: str, side: str, expected_qty: float) -> bool:
        """Verify that a position was actually created on the exchange after order execution."""
        try:
            if not hasattr(self, 'client') or not self.client:
                self.logger.warning("[POSITION_VERIFY] No client available for verification")
                return False
            ...
            for pos in positions:
                if pos.get('symbol') == symbol:
                    ...
                    if pos_amt != 0:
                        ...
                        if actual_side == side:
                            self.logger.info("[POSITION_VERIFY] ✅ Position confirmed: %.3f %s (%s)", 
                                           abs(pos_amt), symbol, side)
                            
                            # Check if quantity matches (allow small differences due to rounding)
                            qty_diff = abs(abs(pos_amt) - expected_qty)
                            if qty_diff > expected_qty * 0.1:  # More than 10% difference
                                self.logger.warning("[POSITION_VERIFY] ⚠️ Quantity mismatch: expected %.3f, got %.3f", 
                                                  expected_qty, abs(pos_amt))
                            return True
            
            self.logger.error("[POSITION_VERIFY] ❌ No position found after order execution!")
            return False
    ...
    async def _check_existing_position(self, symbol: str, side: str) -> bool:
        ...
        try:
            if not hasattr(self, 'client') or not self.client:
                ...
                return symbol in self.active_positions and self.active_positions[symbol].get('side') == side
            ...
            try:
                    ...
                    if pos_amt != 0:
                        ...
                        if pos.get('symbol') == symbol and pos_side == side:
                            ...
                            if abs_pos_amt >= min_threshold:
                                self.logger.debug("[POSITION_CHECK] Found SIGNIFICANT %s position: %.3f %s (>= %.3f threshold)", 
                                                side, abs_pos_amt, symbol, min_threshold)
                                return True
                            else:
                                # Found position but it's too small - consider it dust
                                self.logger.info("[POSITION_DUST] Found tiny %s position: %.6f %s (< %.3f threshold) - IGNORING as dust", 
                                                side, abs_pos_amt, symbol, min_threshold)
                ...
                self.logger.debug("[POSITION_CHECK] Failed to get positions from exchange: %s", e)
    ...
    async def _update_position_pnl(self, symbol: str) -> None:
        ...
        try:
            if not hasattr(self, 'client') or not self.client:
                self.logger.debug("[PNL_UPDATE] No client available, skipping PnL update")
                return
            
            if symbol not in self.active_positions:
                self.logger.debug("[PNL_UPDATE] No active position found for %s", symbol)
                return
            ...
            for pos in positions:
                if pos.get('symbol') == symbol:
                    ...
                    if pos_amt != 0:  # Active position found
                        ...
                        pnl_pct = (unrealized_pnl / (entry_price * abs(pos_amt))) * 100 if entry_price > 0 and pos_amt != 0 else 0
                        
                        self.logger.debug("[PNL_UPDATE] %s: PnL=$%.2f (%.2f%%), Entry=$%.4f, Mark=$%.4f", 
                                        symbol, unrealized_pnl, pnl_pct, entry_price, mark_price)
                        return
            
            # Position not found on exchange - it might have been closed
            self.logger.warning("[PNL_UPDATE] Position for %s not found on exchange - might be closed", symbol)
            ...
            for symbol in list(self.active_positions.keys()):
                ...
                if self.iteration % 100 == 0:  # Every ~100 seconds
                    ...
                    if position:
                        ...
                        pnl_pct = (pnl / (position.get('entry_price', 1) * position.get('quantity', 1))) * 100
                        self.logger.info("[PNL_SUMMARY] %s: $%.2f (%.2f%%)", symbol, pnl, pnl_pct)
    ...
    async def _run_ai_optimization_cycle(self) -> None:
        ...
        try:
            ...
            if self.iteration % 200 == 0:  # Every ~3 minutes
                self.logger.info("🧠 [AI_STATUS] Trades recorded: %d, Learning active: %s", 
                               trade_count, bool(self.adaptive_learning))
            ...
            if trade_count >= 20 and self.iteration % 1000 == 0:  # Every ~15 minutes
                self.logger.info("🎯 [AI_OPTIMIZATION] Running periodic optimization with %d trades", trade_count)
                
                # Run Bayesian optimization
                if hasattr(self.adaptive_learning, 'advanced_ai') and self.adaptive_learning.advanced_ai:
                    optimal_params = await self.adaptive_learning.optimize_parameters_with_ai('sharpe_ratio')
                    
                    if optimal_params:
                        self.logger.info("🎯 [AI_OPTIMIZATION] New optimal parameters found")
                        
                        # Apply optimizations
                        if 'confidence_threshold' in optimal_params:
                            self.logger.info("🎯 [AI_TUNED] Confidence: %.3f", optimal_params['confidence_threshold'])
                        
                        if 'position_size_multiplier' in optimal_params:
                            old_risk = self.risk_pct
                            adjusted_risk = old_risk * optimal_params['position_size_multiplier']
                            self.risk_pct = max(0.1, min(2.0, adjusted_risk))
                            self.logger.info("🎯 [AI_TUNED] Position size: %.2f%% → %.2f%%", old_risk, self.risk_pct)
                        
                        # Log reasoning if available
                        if 'reasoning' in optimal_params:
                            self.logger.info("🧠 [AI_REASONING] %s", optimal_params['reasoning'])
                ...
                if hasattr(self.adaptive_learning, 'analyze_ab_test_results'):
                    ...
                    if ab_results and ab_results.get('winner'):
                        self.logger.info("🧪 [A/B_TEST] Winner variant: %s (confidence: %.2f)", 
                                       ab_results['winner'], ab_results.get('confidence', 0))
                ...
                if hasattr(self.adaptive_learning, 'discover_patterns'):
                    ...
                    if patterns:
                        for pattern in patterns[:3]:  # Log top 3 patterns
                            self.logger.info("🧩 [PATTERN] Found: %s (strength: %.2f)", 
                                           pattern.get('description', 'Unknown'), pattern.get('strength', 0))
        ...
        try:
            if hasattr(self, 'client') and self.client:
                ...
                try:
                    ...
                    self._binance_client = self.client
                    self.logger.info("[BINANCE] Initialized trading client (testnet=%s)", getattr(self.config, 'testnet', True))
                except Exception as init_e:
                    self.logger.error("[BINANCE] Failed to initialize trading client: %s", init_e)
                    self.client = None
            ...
            if self.dca_manager == "pending" and self.client and DCAManager:
                try:
                    ...
                    dca_config = DCAConfig(
                        ...
                        level_spacing_pct=getattr(self.config, 'level_spacing_pct', 1.5),
                        level_multipliers=getattr(self.config, 'level_multipliers', [1.0, 1.5, 2.0, 2.5, 3.0]),
                    ...
                    self.logger.info("[DCA] DCA Manager initialized with Binance client")
                    self.logger.info(f"[DCA] Config: max_levels={dca_config.max_levels}, spacing={dca_config.level_spacing_pct}%")
                except Exception as dca_e:
                    self.logger.warning("[DCA] Failed to initialize DCA Manager: %s", dca_e)
                    self.dca_manager = None
            
            # Initialize Trailing Stop Manager with Binance client
            if self.use_trailing_stop and self.client:
                try:
                    from exchange.trailing_stop import TrailingStopManager, TrailingStopConfig
                    ...
                    self.logger.warning("[TRAIL_SL] Failed to initialize trailing stop manager: %s", trail_e)
                    self.trailing_stop_manager = None
            ...
            if self.client and PortfolioTracker:
                try:
                    ...
                    self.logger.info("[PORTFOLIO] Portfolio Tracker initialized - Balance/P&L monitoring enabled")
                except Exception as pt_e:
                    self.logger.warning("[PORTFOLIO] Failed to initialize Portfolio Tracker: %s", pt_e)
                    self.portfolio_tracker = None
            else:
                self.portfolio_tracker = None
            ...
            if self.client:
                try:
                    from exchange.exit_tracker import create_exit_tracker, integrate_exit_tracker_callbacks
                    
                    self.exit_tracker = await create_exit_tracker(self.client, self.adaptive_learning)
                    integrate_exit_tracker_callbacks(self.exit_tracker, self)
                    
                    self.logger.info("🎯 [EXIT_TRACKER] Real-time TP/SL tracking initialized")
                    
                except Exception as et_e:
                    self.logger.warning("🎯 [EXIT_TRACKER] Failed to initialize exit tracker: %s", et_e)
    ...
    async def _setup_tp_sl_orders(self, symbol: str, side: str, qty: float, entry_price: float, strength: float) -> None:
        ...
        try:
            ...
            tp_levels, sl_level = self._calculate_tp_sl_levels(entry_price, side, strength)
            ...
            success = await self._try_advanced_exit_setup(symbol, side, qty, entry_price, sl_level, tp_levels)
            
            if not success:
                # Fallback to direct order placement
                self.logger.info("[TP_SL] Using direct order placement (advanced exits not available)")
            ...
            self.logger.info("[TP_SL] Setup complete for %s: %d TP levels, SL at %.2f", 
                           symbol, len(tp_levels), sl_level or 0.0)
            ...
            if self.trailing_stop_manager and tp_levels and sl_level:
                try:
                    ...
                    self.logger.warning("[TRAIL_SL] Failed to register position: %s", trail_e)
                
        except Exception as e:
            self.logger.error("[TP_SL_FAILED] Failed to setup TP/SL for %s: %s", symbol, e)

    async def _try_advanced_exit_setup(self, symbol: str, side: str, qty: float, entry_price: float, 
                                       sl_level: float, tp_levels: list[float]) -> bool:
        """Try to use advanced exit management from monolith."""
        try:
            # Import advanced exit functions from monolith
            from exchange.exits import ensure_exits_on_exchange, ensure_sl_on_exchange, ensure_tp_on_exchange
            ...
            tp_shares = [0.4, 0.35, 0.25] if len(tp_levels) == 3 else [1.0 / len(tp_levels)] * len(tp_levels)
    ...
    def _calculate_tp_sl_levels(self, entry_price: float, side: str, strength: float) -> tuple[list[float], float]:
        ...
        try:
            ...
            config_tp_levels = self.config.parse_tp_levels()  # Get from config: [1.5, 3.0, 5.0]
            ...
            symbol = getattr(self, '_current_symbol', 'BTCUSDT')  # Get current symbol for rounding
            ...
            self.logger.info("[TP_SL_CALC] 🎯 %s %dx: Entry=$%.4f, SL=$%.4f(-%.1f%%), TPs=%s", 
                           side, leverage, entry_price, sl_level, sl_pct,
                           [f"${tp:.4f}({profit:.1f}%)" for tp, profit in zip(tp_levels, profit_pcts)])
            
            # Verify all levels are different and valid
            if len(set(tp_levels)) != len(tp_levels):
                self.logger.warning("[TP_SL_CALC] ⚠️ Some TP levels are identical after rounding!")
            ...
            default_tps = [entry_price * 1.015, entry_price * 1.03, entry_price * 1.05] if side.upper() == "BUY" else [entry_price * 0.985, entry_price * 0.97, entry_price * 0.95]
            default_sl = entry_price * 0.98 if side.upper() == "BUY" else entry_price * 1.02
            ...
            for order in open_orders:
                ...
                if is_exit_order:
                    try:
                        ...
                        cancelled_count += 1
                        self.logger.info("[EXIT_CANCEL] ✅ Cancelled %s order %s (reduceOnly=%s)", 
                                       order_type, order['orderId'], order.get('reduceOnly', False))
                    except Exception as e:
                        self.logger.warning("[EXIT_CANCEL_FAILED] ❌ Could not cancel order %s: %s", order['orderId'], e)
                else:
                    self.logger.debug("[EXIT_SKIP] Skipping non-exit order %s (type=%s)", order['orderId'], order_type)
            
            if cancelled_count > 0:
                self.logger.info("[EXIT_CANCEL] Cancelled %d existing exit orders for %s", cancelled_count, symbol)
                
        except Exception as e:
            self.logger.warning("[EXIT_CANCEL_FAILED] Failed to cancel exit orders for %s: %s", symbol, e)
    
    async def _place_stop_loss_order(self, symbol: str, side: str, qty: float, sl_price: float) -> None:
        ...
        try:
            ...
            self.logger.info("[SL_ORDER] Placing %s STOP_MARKET (closePosition): %s @ %.2f", 
                           sl_side, symbol, rounded_sl_price)
            
            if hasattr(self, 'client') and self.client:
                # Skip time sync - method not available in BinanceClient
                # Binance client handles timestamp internally
                self.logger.debug("[TIME_SYNC] Using client internal timestamp handling")
                ...
                self.logger.info("[SL_SUCCESS] Stop loss placed: %s (closePosition=true)", 
                               sl_result.get('orderId', 'N/A'))
                
        except Exception as e:
            self.logger.error("[SL_FAILED] Failed to place stop loss: %s", e)

    async def _place_take_profit_orders(self, symbol: str, side: str, total_qty: float, tp_levels: list[float]) -> None:
        """Place multiple take profit orders with quantity distribution."""
        try:
            # Distribute quantity across TP levels (per symbol precision)
            tp_quantities = self._distribute_tp_quantities(total_qty, len(tp_levels), symbol)
            ...
            for i, (tp_price, tp_qty) in enumerate(zip(tp_levels, tp_quantities)):
                try:
                    ...
                    self.logger.info("[TP_ORDER] Placing %s take profit %d: %.3f %s @ %.2f", 
                                   tp_side, i+1, rounded_tp_qty, symbol, rounded_tp_price)
                    ...
                    if hasattr(self, 'client') and self.client:
                        ...
                        self.logger.info("[TP_SUCCESS] Take profit %d placed: %s", 
                                       i+1, tp_result.get('orderId', 'N/A'))
                        
                except Exception as e:
                    self.logger.error("[TP_FAILED] Failed to place take profit %d: %s", i+1, e)
                    
        except Exception as e:
            self.logger.error("[TP_ORDERS_FAILED] Failed to place take profit orders: %s", e)

    def _distribute_tp_quantities(self, total_qty: float, num_levels: int, symbol: str = "BTCUSDT") -> list[float]:
        ...
        self.logger.info("[TP_DIST] Quantity distribution: %s (total: %.3f)", 
                       [f"{q:.3f}" for q in quantities], sum(quantities))
    ...
    def _calculate_tp_sl_levels(self, entry_price: float, side: str, signal_strength: float) -> tuple:
    ...
    async def _handle_dca_trigger(self, symbol: str, position: Dict) -> None:
        ...
        try:
            ...
            if not self.dca_manager or not hasattr(self.dca_manager, 'active_positions'):
            ...
            await self._place_order(symbol, side, dca_qty, current_price, 0.8)  # Lower strength for DCA
            
            # Update position tracking
            self.active_positions[symbol] = {
                'side': side,
                'entry_price': (entry_price * quantity + current_price * dca_qty) / (quantity + dca_qty),  # New weighted average
            ...
            self.logger.info("[DCA_SUCCESS] DCA order placed for %s: %.3f @ %.2f (total qty: %.3f)", 
                           symbol, dca_qty, current_price, quantity + dca_qty)
            
        except Exception as e:
            self.logger.error("[DCA_FAILED] Failed to handle DCA trigger for %s: %s", symbol, e)
                ...
                if pos['symbol'] == symbol:
                    ...
                    if abs(position_amt) > 0:  # Active position
                        ...
                        if symbol in self.active_positions:
                            ...
                            self.logger.debug(f"[PNL_UPDATE] {symbol}: {unrealized_pnl:+.2f} USDT @ {mark_price:.4f}")
                        break
    ...
    async def _update_all_positions_pnl(self) -> None:
        ...
        try:
            ...
            for pos in positions:
                ...
                if abs(position_amt) > 0 and symbol in self.active_positions:
                    unrealized_pnl = float(pos.get('unRealizedProfit', 0))
                    mark_price = float(pos.get('markPrice', 0))
                    entry_price = float(pos.get('entryPrice', 0))
                    ...
                    if self.dca_manager:
                        await self.dca_manager.check_dca_trigger(symbol, self.active_positions[symbol])
    ...
    async def _run_ai_optimization_cycle(self) -> None:
        ...
        try:
            ...
            if hasattr(self.adaptive_learning, 'advanced_ai') and self.adaptive_learning.advanced_ai:
                ...
                recommendations = await self.adaptive_learning.get_advanced_ai_recommendations(market_data)
                
                if recommendations and recommendations.get('confidence', 0) > 0.5:
                    self.logger.info(f"🧠 [AI_CYCLE] Applied recommendations with {recommendations['confidence']:.2f} confidence")
    ...
    async def _check_emergency_stop_loss(self) -> bool:
        ...
        try:
            ...
            if self.initial_equity > 0:
                loss_pct = ((current_equity - self.initial_equity) / self.initial_equity) * 100
            ...
            self.logger.error(f"[EMERGENCY_SL] Failed to check emergency stop loss: {e}")
            return False
    ...
    async def _initialize_advanced_ai(self) -> None:
        ...
        try:
            if not self.adaptive_learning or not self.adaptive_learning.advanced_ai:
                logger.debug("🧠 [ADVANCED_AI] Not available - basic adaptive learning only")
                return
            
            logger.info("🧠 [ADVANCED_AI] Initializing advanced intelligence features...")
            ...
            if recommendations and recommendations.get('confidence', 0) > 0.5:
                logger.info(f"🎯 [AI_RECOMMENDATIONS] Confidence: {recommendations['confidence']:.2f}")
                ...
                for param, value in recommendations.get('recommendations', {}).items():
                    if param == 'confidence_threshold':
                        ...
                        self.risk_pct = max(0.1, min(2.0, adjusted_risk))
                        logger.info(f"🎯 [AI_TUNING] Position sizing: {self.risk_pct:.2f}%")
            ...
            if hasattr(self.adaptive_learning, 'trades_history') and len(self.adaptive_learning.trades_history) >= 20:
    ...
    async def _start_advanced_ab_testing(self, variants: List[Dict]) -> None:
        ...
        try:
            ...
            logger.info(f"🧪 [ADVANCED_AB] Starting testing with {len(variants)} variants")
            
            result = await self.adaptive_learning.start_advanced_ab_testing(variants)
            
            if result.get('test_started'):
                logger.info(f"🧪 [ADVANCED_AB] Test started: {result['current_allocation']} allocation")
                logger.info(f"🧪 [ADVANCED_AB] Min trades required: {result['min_trades_required']}")
    ...
    async def _run_periodic_optimization(self) -> None:
        ...
        try:
            ...
            logger.info("🎯 [AI_OPTIMIZATION] Running Bayesian parameter optimization...")
            
            optimal_params = await self.adaptive_learning.optimize_parameters_with_ai('sharpe_ratio')
            
            if optimal_params:
                logger.info("🎯 [AI_OPTIMIZATION] Optimization complete - parameters updated")
                
                # Apply optimized parameters
                if 'confidence_threshold' in optimal_params:
                    logger.info(f"🎯 [OPTIMIZED] Confidence: {optimal_params['confidence_threshold']:.3f}")
                
                if 'position_size_multiplier' in optimal_params:
                    adjusted_risk = self.risk_pct * optimal_params['position_size_multiplier']
                    self.risk_pct = max(0.1, min(2.0, adjusted_risk))
    ...
    async def _generate_learning_visualization(self) -> None:
        ...
        try:
            if not self.adaptive_learning or not hasattr(self.adaptive_learning, 'learning_visualizer'):
                return
            ...
            if self.iteration % 600 == 0:  # Every 10 minutes
                ...
                if dashboard_path:
                    self.logger.info(f"📊 [DASHBOARD] Updated learning dashboard: {dashboard_path}")
            
            # Log learning status for immediate visibility
            if snapshot.adaptations_count > 0:
                self.logger.info("📊 [LEARNING_STATUS] ========================================")
                self.logger.info(f"🔧 Total Adaptations: {snapshot.adaptations_count}")
                self.logger.info(f"🎯 Current Confidence: {snapshot.confidence_threshold:.3f}")
                self.logger.info(f"📏 Position Multiplier: {snapshot.position_size_multiplier:.2f}x")
                self.logger.info(f"🔄 DCA Enabled: {'✅' if snapshot.dca_enabled else '❌'}")
                
                if snapshot.total_trades > 0:
                    self.logger.info(f"📈 Performance: {snapshot.total_trades} trades, {snapshot.win_rate:.1%} WR, ${snapshot.total_pnl:+.2f} PnL")
